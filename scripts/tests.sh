...
if [[ ${target} == "hello-world" ]] || [[ ${target} == "ALL" ]]; then
    test_run=1
    IMAGE_NAME="${TEST_IMAGE_BASE}"-hello-world"${ARCH}":"${PYTHON_MINOR}-${OS_VERSION}-${DEBIAN_NAME}-${CI_PIPELINE_ID}"
...
if [[ ${target} == "gunicorn" ]] || [[ ${target} == "ALL" ]]; then
    test_run=1
    IMAGE_NAME="${TEST_IMAGE_BASE}"-gunicorn"${ARCH}":"${PYTHON_MINOR}-${OS_VERSION}-${DEBIAN_NAME}-${CI_PIPELINE_ID}"
...
if [[ ${target} == "fastapi" ]] || [[ ${target} == "ALL" ]]; then
    test_run=1
    IMAGE_NAME="${TEST_IMAGE_BASE}"-fastapi"${ARCH}":"${PYTHON_MINOR}-${OS_VERSION}-${CI_PIPELINE_ID}"
...
if [[ ${target} == "pandas" ]] || [[ ${target} == "ALL" ]]; then
    test_run=1
    IMAGE_NAME="${TEST_IMAGE_BASE}"-pandas"${ARCH}":"${PYTHON_MINOR}-${OS_VERSION}-${DEBIAN_NAME}-${CI_PIPELINE_ID}"
...
if [[ ${target} == "google-cloud" ]] || [[ ${target} == "ALL" ]]; then
    ...
    IMAGE_NAME="${TEST_IMAGE_BASE}"-google-cloud"${ARCH}":"${PYTHON_MINOR}-${OS_VERSION}-${DEBIAN_NAME}-${CI_PIPELINE_ID}"
...
if [[ ${target} == "kubernetes" ]] || [[ ${target} == "ALL" ]]; then
    ...
    IMAGE_NAME="${TEST_IMAGE_BASE}"-kubernetes"${ARCH}":"${PYTHON_MINOR}-${OS_VERSION}-${DEBIAN_NAME}-${CI_PIPELINE_ID}"
