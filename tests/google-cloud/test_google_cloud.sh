...
function test_google_cloud_image() {
    ...
    docker run --rm --name=pubsub-emulator --network=distroless -d -p 127.0.0.1:8085:8085/tcp al3xos/pubsub-emulator:latest

    _console_msg "Giving time for emulator to start ..." INFO
    sleep 10
